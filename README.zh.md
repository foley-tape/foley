...
**隐私纪律**：只读本地会话日志并**蒸馏**为事件骨架（动词/时刻/量级/哈希目标）——工具输入与对话内容永不落盘；失败步只留**脱敏错误类**（凭据/路径/令牌/邮箱抹为占位符）供聚类；零遥测、全离线。**蒸馏带默认即全脱敏**（时间相对化＋非内建工具名加盐哈希＋不指纹化源文件；`--raw` 仅本机调试且强制警示）；导出 mp4 抹墙钟、dub 边车不落日期（G7 闸，M2.6）。
