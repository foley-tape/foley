...
# REC：顶中独居·全脸入画+负空间（z 1.92→2.05 上移一丝·独居半径内无一物）
cyl('rec', 0.11, 0.10, (0.0, -0.06, 2.05), M_rec)
...
# 计数轮：盘区右上小窗（§三迁位·数的是介质属走带时态；屏右=世界 −X）
cube('counter', (0.52, 0.06, 0.26), (-2.55, -0.04, 1.80), M_dark)
cube('counter_win', (0.42, 0.02, 0.16), (-2.55, 0.02, 1.80), M_mid)
...
cube('vuface', (1.30, 0.06, 0.60), (-2.4, -0.02, -1.12), M_vu)          # VU：主仪表之右（屏右=−X）
for i, lz in enumerate((-0.88, -1.14, -1.40)):                           # 灯组三窗：主仪表之左
    cube(f'lampw{i}', (0.31, 0.05, 0.095), (0.66, -0.03, lz), M_lamp if i == 0 else M_dark)
# 主功能选择器（§四）：机加工旋钮三档 OFF·TEST·ON（拨杆+PLAY 圆顶处决·不建）
cyl('selector', 0.24, 0.14, (1.35, -0.04, -1.14), M_hi)
cube('sel_ptr', (0.035, 0.04, 0.20), (1.35, 0.05, -1.05), M_dark)       # 指针帽线（浮出旋钮前脸）
for a, dz in ((-0.32, 0.30), (0.0, 0.34), (0.32, 0.30)):                 # 三档刻度短线
    cube(f'sel_tick{a}', (0.02, 0.02, 0.07), (1.35 + a * 0.28, -0.05, -1.14 + dz * 0.62), M_mid)
# 走纸记录仪（今晚的历史·机身之底）+滑针
...
def spines(shelf_z, mix, gap_at=None):
    x = -4.05
    n = 0
    while x < 4.1:
        ...
        o = cube(f'sp{shelf_z}{n}', (w, 0.86, h), (x + w / 2, 0.10, shelf_z + 0.045 + h / 2), m)
        o.rotation_euler[1] = lean
        x += w + random.uniform(0.015, 0.05); n += 1
spines(SHELF_Z[0], [7, 5, 7], gap_at={5, 14})                            # 首层：常用尺寸·两空位
spines(SHELF_Z[1], [7, 10.5, 5], gap_at={9})                             # 中层：混排
spines(SHELF_Z[2], [10.5, 7, 10.5], gap_at=set())                        # 深层：大盘+一只白净金属巨盘（#11）
# 鞋盒（最底·封存的历史·最深暗场）
for i, (bx, bz, bw) in enumerate(((-2.2, -9.6, 2.6), (1.4, -9.8, 2.2), (3.3, -9.5, 1.4))):
    cube(f'shoebox{i}', (bw, 1.6, 1.0), (bx, 0.35, bz), M_dark)
cube('floor', (9.0, 2.4, 0.1), (0, 0.6, -10.55), M_wood)
...
FRAME_A = (2.25, -3.95)    # 默认取景框=hero（架沿露 (3.95-3.55)/6.2≈10%vh）
FRAME_B = (-3.95, -11.0)   # 带库+鞋盒段（接缝藏架沿下暗区）
...
if SEG in ('b', 'all'):
    cam.location = (-0.2, 12.6, -7.2); tgt.location = (-0.2, 0.05, -8.0)   # 同俯角平移=镜头下摇
    solve_frame(*FRAME_B)
