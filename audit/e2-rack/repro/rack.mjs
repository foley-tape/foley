...
const sig = (page) => page.evaluate(() => ({
  ...
  playState: document.getElementById('ctl-play')?.dataset.state,
  playDisabled: document.getElementById('ctl-play')?.disabled,
  ...
  engineId: window.__stage?.sound ? (window.__eid ??= Symbol(), window.__stage.sound.engine.__id ??= Math.random()) : null,
}));
...
// [D] 控制面板 play/pause（rule 4）
await p1.evaluate(() => document.getElementById('ctl-play').click()); await p1.waitForTimeout(500);
const paused = await sig(p1);
await p1.evaluate(() => document.getElementById('ctl-play').click()); await p1.waitForTimeout(500);
const resumed = await sig(p1);
await p1.screenshot({ path: join(here, '..', 'shots', 'rack-playing.png') });
...
const V = {
  A_landing_empty: { pass: A.phase === 'EMPTY' && A.selected === null && A.cassettes > 3 && A.playDisabled === true, ...A },
  B_cueing_locks: { pass: cueing.phase === 'CUEING' && cueing.playDisabled === true && cueing.dubLocked === true, phase: cueing.phase, dubLocked: cueing.dubLocked },
  B_playing: { pass: playing.phase === 'PLAYING' && playing.selected === 'storm' && playing.playState === 'playing', selected: playing.selected },
  ...
  D_panel_pause: { pass: paused.phase === 'PAUSED' && paused.playState === 'paused', phase: paused.phase },
  D_panel_resume: { pass: resumed.phase === 'PLAYING', phase: resumed.phase },
  E_restart_empty: { pass: afterRestart.phase === 'EMPTY' && afterRestart.selected === null, ...afterRestart },
