...
1. **包体预算 = 2MB**：席一定数 == 席三闸默认（`FOLEY_PACK_BUDGET_KB` 可调）——**已统一**，无分歧。
2. **fixtures 断言**：`check-pack-budget.mjs` 增「任何 `fixtures/` 漏入包即红」——统一席一 item2「无任何 fixture 在包」验收口径，补 `check-heavy-media` 文本白名单盲区（captain.curve.csv 13MB 文本曾整条溜过）。
...
- 席一 item 6「签名绑哈希」规则成文 → 归席三诚约闸（本阶段续做）。
...
**判词：席一工单 1／2 通过对审，候操作员合入 `seat/trust`→`main`。合入后 `prepublishOnly` 三闸生效、README对表/包体预算随之转绿。**
