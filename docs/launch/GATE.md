...
### 7.1 待架构师裁（口径冲突，勿自调，已上报）

1. **音频开箱声**：§0.3 令音频（唱片 mp3 ＋ 床音 wav）全走 Releases、不入 tarball；工具箱口径「decline → room tone」。现实：干净装无 wav → 走 **合成织体**退路（§C 设计），非真 `room-tone.wav`。**裁**：接受合成退路为开箱声（README 现按此写「ambient bed by default」）？或把轻量床音（~2.9MB wav）纳入 `files` 换真 room tone（tarball 905kB→~3.8MB，仍秒级）？
2. **D-1／D-0 时序**：公开镜像（D-1 推）先于 `npm publish`（D-0）→ 公开 README 的 `npx foley` 有数小时为假窗口。**荐**：`publish` 与镜像推送同时或前置，避免「发布时刻为真」被时序破。
