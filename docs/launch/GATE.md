...
> ⚠ **2026-07-15 · NIGHT-3 超覆盖（治理真话·席三工单3）**：本记分牌为 M2.6（末更 2026-07-07）态，**已被 NIGHT-3 双盲夜审超覆盖**——两耳独立判词皆 **NO-GO 公开发布／可继续内部体验**，四件发布阻断 P0 复现（假入口／隐私契约「never stored」自相矛盾 vs 本地 `rack.json`／零会话死 LIVE／死 Claude 仍亮 REC）。**本表 pre-NIGHT-3 的 ✅ 一律按存疑读**（尤 §3 冷读 #6「never stored」——两域法尚未成文·随席一 README 合入＋工单4 修宪同批裁）；当前权威门禁台账＝`FOLEY_LEDGER.md §二·七`（发现逐条见 §四·五「NIGHT-3」块·报告存 `audit/night3-{L,R}/`）。发布扳机与品味门权属船长。
...
| G8 | **零配置第一分钟有声**（新增） | 🟠 **热修落地·候船长复走** | 船长物理走查实证三根断线（`npx .` 尾随歇场会话→针死零/纸平/死寂）：①正页从未接声桥（soundbridge 只在 demo 页服役）——main.js 现挂 SoundBridge，首次手势解锁、与视觉同起点、`?sound=0` 关；②serve 静态根钉死 stage/，声资产全 404——现挂 `/sound-params.json`+`/sound/**`+`/records/**`（MIME 补 mp3/wav，fence 防穿越）；③空盘/歇场（>15min）自举死寂——**〔NIGHT-3 治理真话 2026-07-15 订正：此描述已过时〕** 现代码 `serve.mjs:535-536` **不再 302 落演示卷**：裸正门直上 index.html 空载卡带架，「死机观感」由架上有带可选天然化解（`?mode=live` 来意尊重；`FOLEY_PROJECTS` 供 CI 模拟空盘）。声桥双退路在册（唱片缺→房间层，织体缺→合成，graph.js M2.4 §C）。金测试 `g8.bootstrap.test.ts` ×3。**已知界（如实）**：live 流式声部件候 Track-SOUND；回放视觉倍速 vs 压缩轴 1× 的画声速率不齐属在案 known-limit（轴主归一记案）。**候船长复走签验收记**。 |
...
G8 零配置第一分钟有声 🟠 热修落地·候船长复走 （正页接声桥+serve 供声粮+空盘卡带架〔不再 302·NIGHT-3 更〕 · 3 条金测试 · 候船长验收记）
```
