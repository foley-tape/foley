...
test('LIVE-2 两条通道：乐音级量化到下一网格（宁迟勿早）；呼唤级 ASK 直通不量化', () => {
  ...
  for (const m of ['hum', 'hiss', 'crackle'] as const) r.eng.setMute(m, true);
  r.feed(0.05, 22, 0.1);
...
test('LIVE-3 唱片全程：热装即上桥；DONE 滑停真静默；非 DONE 相到达=复活重落针', () => {
  ...
  for (const m of ['hum', 'hiss', 'crackle', 'fg'] as const) r.eng.setMute(m, true);
...
test('LIVE-6 落针宣告（己-5 合龙微单）：wired→needleDrop 出声；房间层态（无唱片）照响；fg mute 静默', () => {
  // ① 无唱片（房间层态）：落针必须可闻——接线宣告与唱片在否无关（走 fgBus 非唱片链）。
  //    hardMute 即时零化全部床 level（不含 fgBus），裸听落针；落针路 envG→fgBus 不受 hardMute。
  const r = rig(); // records=null
  r.eng.hardMute();
  r.eng.needleDrop(0.2);
  const wav = r.ctx.render(1.0);
  const onset = onsetAt(wav, EAR_SR, 0.1, 0.6, 0.004);
  assert.ok(onset >= 0.2 && onset < 0.28, `落针应在排程刻 0.2s 附近出声，实测 ${onset.toFixed(3)}`);
  const drop = rmsDb(wav, EAR_SR, 0.2, 0.45);
  assert.ok(drop > -40, `落针应有声（房间层态，无唱片），实测 ${drop.toFixed(1)} dBFS`);
  const after = rmsDb(wav, EAR_SR, 0.6, 0.95);
  assert.ok(after < -55, `落针是一次性源，尾巴应落回静默，实测 ${after.toFixed(1)} dBFS`);

  // ② fg 隔离板勾掉 → 落针连带静默（announcement 属前景族，与 pluck/page 同门）
  const r2 = rig();
  r2.eng.hardMute();
  r2.eng.setMute('fg', true);
  r2.eng.needleDrop(0.2);
...
test('LIVE-8 ASK 重唤（席二工单1·状态契约 R2 声学半）：等待期每 askRepeatSec 重奏动机——呼吸整个等待期的声之形', () => {
  // 无唱片 rig（合成退路床）：动机尖峰在安静等待床上可分辨；pendingAsk 流不断
  const r = rig();
  // 头 5s 正常 WORKING 流热身
  r.feed(0.05, 5, 0.05);
  // t=5：ASK 到达（呼唤级直通首唤·graph:758 置 lastAskRepeat）
  r.at(5.0); r.bridge.onMoment({ t: 5000, verb: 'ASK', outcome: 'NA', tags: [] });
  // 等待期：pendingAsk=true 的 WAITING 流一路喂到 100s（agent 停手·机器陪等）
  r.feed(5.05, 100, 0.05, { phase: 'WAITING', pendingAsk: true, needle: 0.1, A: 0.05 });
  const wav = r.ctx.render(100);
  // 静等基线窗（首唤余音已尽、重唤未至）vs 首唤/重唤紧窗（motif 两音 ~1.2s；宽窗会被床稀释）
  // 阈值 +3dB 系标定值（实测首唤 +5.2 / 重唤 +4.1，留余量）——非品味，是"到场且可辨"的机器判据
  const quiet = rmsDb(wav, EAR_SR, 60, 88);
  const first = rmsDb(wav, EAR_SR, 5.0, 6.2);
  const recall = rmsDb(wav, EAR_SR, 94.5, 96.2);   // lastAskRepeat=5s + askRepeatSec(90s) → ~95s
  assert.ok(first > quiet + 3, `首唤动机应高于静等床（Δ=${(first - quiet).toFixed(1)}dB，标定 +5.2）`);
  assert.ok(recall > quiet + 3, `90s 重唤应到场且可辨（Δ=${(recall - quiet).toFixed(1)}dB，标定 +4.1）`);
  // 提前唤=0 帐：askRepeatSec 之前的长静等窗必须与基线同级（等待不是骚扰）
  const early = rmsDb(wav, EAR_SR, 10, 60);
  assert.ok(early < quiet + 1.5, `askRepeatSec 前不得提前重唤（early=${early.toFixed(1)} vs quiet=${quiet.toFixed(1)}）`);
  // 事后归静帐：重唤余音散尽回到等待床
  const after = rmsDb(wav, EAR_SR, 98, 100);
  assert.ok(after < quiet + 1.5, `重唤后回归静等（after=${after.toFixed(1)}）`);
});
