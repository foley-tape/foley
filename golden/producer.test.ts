...
// ATF 重建后 spool 行携身份账（incarnation＋producerEpoch＋eventId·新 hook 同款）；
// 无身份 v:1 遗留行被生命周期安全忽略（ATF §5），故金测激励一律走 v:2 身份行。
function spoolStartLine(
  home: string, transcript: string, pid: number | null, pidCommand: string | null,
  incarnation = 'inc-1', producerEpoch = 1,
): void {
  const dir = join(home, 'spool');
  mkdirSync(dir, { recursive: true });
  appendFileSync(join(dir, 'events.ndjson'), JSON.stringify({
    v: 2, at: Date.now(), eventId: randomUUID(), kind: 'session-start', sessionId: 'prod-test',
    transcriptPath: transcript, source: 'startup', incarnation, producerEpoch, pid, pidCommand,
  }) + '\n');
}
...
test('PROD-5 收工即熄（ended）：session-end 行到 → producer=ended——SIGTERM 优雅退同路（善终非死相）', async () => {
  ...
  try {
    ...
    // session-end 行（钩子在 SIGTERM/自然完成时都会发）——身份四元组全匹配（key+sessionId+incarnation+epoch）
    appendFileSync(join(rig.home, 'spool', 'events.ndjson'), JSON.stringify({
      v: 2, at: Date.now(), eventId: randomUUID(), kind: 'session-end', sessionId: 'prod-test',
      transcriptPath: realpathSync(rig.tape), reason: 'other', incarnation: 'inc-1', producerEpoch: 1,
    }) + '\n');
...
test('PROD-6 终态持久＋复活（席一 D2 复审 #3）：session-end→ended 持久不自退回 null；同 transcript 新 session-start→复活 alive', async () => {
  ...
  try {
    ...
    appendFileSync(join(rig.home, 'spool', 'events.ndjson'), JSON.stringify({ v: 2, at: Date.now(), eventId: randomUUID(), kind: 'session-end', sessionId: 'prod-test', transcriptPath: realpathSync(rig.tape), reason: 'other', incarnation: 'inc-1', producerEpoch: 1 }) + '\n');
    ...
    const v2 = spawn('sleep', ['300'], { stdio: 'ignore' });   // 复活：同 transcript 新 session-start（新 incarnation＋更大 epoch）
    spoolStartLine(rig.home, realpathSync(rig.tape), v2.pid!, psCommand(v2.pid!), 'inc-2', 2);
...
test('PROD-8 代际隔离（席一 D2 复审二·issue 1）：会话 A 死后 B 上代际复活，旧 A 的 session-end 绝不覆盖新 B 的 alive', async () => {
  ...
  try {
    ...
    const ev = (o: Record<string, unknown>) => appendFileSync(join(rig.home, 'spool', 'events.ndjson'), JSON.stringify({ v: 2, at: Date.now(), eventId: randomUUID(), ...o }) + '\n');
    ev({ kind: 'session-start', sessionId: 'sess-A', transcriptPath: tk, source: 'startup', incarnation: 'inc-A', producerEpoch: 1, pid: vA.pid, pidCommand: psCommand(vA.pid!) });
    assert.ok(await waitProducer(rig, 'alive', 8000), 'A 报到 alive');
    vA.kill('SIGKILL');
    assert.ok(await waitProducer(rig, 'dead', 8000), 'A 猝死 dead');
    // B 上机（同 transcript·新 PID·新 sessionId）→ 新代际·复活 alive（B 的 session-start 废除 A 的 terminal）
    const vB = spawn('sleep', ['300'], { stdio: 'ignore' });
    ev({ kind: 'session-start', sessionId: 'sess-B', transcriptPath: tk, source: 'startup', incarnation: 'inc-B', producerEpoch: 2, pid: vB.pid, pidCommand: psCommand(vB.pid!) });
    assert.ok(await waitProducer(rig, 'alive', 8000), 'B 上代际 → 复活 alive');
    // 迟到的 A 的 session-end（旧身份四元组）——代际隔离，绝不熄 B
    ev({ kind: 'session-end', sessionId: 'sess-A', transcriptPath: tk, reason: 'other', incarnation: 'inc-A', producerEpoch: 1 });
