...
function mkF(over: Record<string, unknown> = {}) {
  return {
    ...
    storm: false, fails: 2, distinctSigs: 2, maxSameSigRepeat: 1, files: 3,
...
test('65 优先序原文：粗混＞抢救＞一条过＞跳针＞长版＞即兴＞小样＞样片', () => {
  ...
  // 收尾态＞戏剧：张力未解压过跳针
  assert.equal(judgeSeal(mkF({ tEndMean: 0.6, maxSameSigRepeat: 5 })).id, 'ROUGH_MIX');
  // 戏剧＞形状：抢救压过跳针
  assert.equal(judgeSeal(mkF({ tHighDwellS: 200, tEndMean: 0.1, maxSameSigRepeat: 5, tPeak: 0.9 })).id, 'SALVAGE');
  // 稀有先于速写：五分钟零瑕疵是一条过，不是小样
  assert.equal(judgeSeal(mkF({ durS: 300, fails: 0, tPeak: 0.3 })).id, 'ONE_TAKE');
  // 形状＞规模：跳针压过长版
  assert.equal(judgeSeal(mkF({ durS: 12000, maxSameSigRepeat: 3, tPeak: 0.6 })).id, 'LOCKED_GROOVE');
  // 长版＞即兴：三小时多文件也先是长版
...
test('66 判据边界原文锚：报错≤1／同签名≥3／≤10min／≥180min／JAM 三条件', () => {
  ...
  // LOCKED GROOVE：同签名错误重复 ≥3（011-R 原文）——2 不够；异签名 3 错不算
  assert.equal(judgeSeal(mkF({ maxSameSigRepeat: 3, tPeak: 0.6 })).id, 'LOCKED_GROOVE');
  assert.notEqual(judgeSeal(mkF({ maxSameSigRepeat: 2, tPeak: 0.6 })).id, 'LOCKED_GROOVE');
  assert.notEqual(judgeSeal(mkF({ fails: 3, distinctSigs: 3, maxSameSigRepeat: 1, tPeak: 0.6 })).id, 'LOCKED_GROOVE');
