...
import {
  ...
  degreeOf, rootMidiOf, askMotifHz, pearson,
...
test('㉚ 床映射律 v3（声资产批定稿§三·状态表）：哼恒在·嘶带走门控随速度·DONE 嘶止哼存·唱片在位床近隐', () => {
  const base = bedTargets(st({ T: 0.5 }), sp);
  const trim = Math.pow(10, sp.bed.trimDb / 20);
  // 哼：机器上电即在——相位/张力/带走与否皆不动它（呼吸级地板）
  assert.ok(Math.abs(base.hum - trim * sp.bed.humGain) < 1e-12, '哼=humGain（含总闸）');
  assert.equal(bedTargets(st({ T: 0.9, phase: 'IDLE' }), sp).hum, base.hum, 'IDLE 哼同值');
  assert.equal(bedTargets(st({ phase: 'DONE' }), sp).hum, base.hum, 'DONE 哼存（听得见的安静·收工≠关机）');
  // 嘶：带走门控——moving:false（暂停抬带）即止；DONE（滑停带停）即止
  assert.ok(base.hiss > 0, '带走=哼+嘶');
  assert.equal(bedTargets({ ...st({ T: 0.5 }), moving: false }, sp).hiss, 0, '暂停抬带=嘶止');
  assert.equal(bedTargets(st({ phase: 'DONE' }), sp).hiss, 0, 'DONE 带停=嘶止');
  // 嘶随速度幂律单调
  const s1 = bedTargets({ ...st({}), speed: 1 }, sp).hiss;
  const s8 = bedTargets({ ...st({}), speed: 8 }, sp).hiss;
  assert.ok(s8 > s1, '倍速回放嘶更亮（speed^pow）');
  assert.ok(Math.abs(s8 / s1 - Math.min(Math.pow(8, sp.bed.hissSpeedPow), sp.bed.hissSpeedMax)) < 1e-9, '幂律精确（含饱和上限）');
  // 嘶随 wow 微摆（读头接触不稳）
  assert.ok(bedTargets(st({ wow: 1 }), sp).hiss > bedTargets(st({ wow: 0 }), sp).hiss, 'wow↑嘶微涨');
  // crackle 随 T（磨损身份原样）+ 滤波/搁架 T 律原样
  const lo = bedTargets(st({ T: 0.1 }), sp), hi = bedTargets(st({ T: 0.9 }), sp);
  assert.ok(hi.crackle > lo.crackle && hi.filterHz < lo.filterHz && hi.hfShelfDb < lo.hfShelfDb, 'T↑ → 糙、暗');
  assert.ok(bedTargets(st({ pendingAsk: true }), sp).hover, 'WAITING hover 位保留');
  // 能量单调随 T 仍成立（crackle 扛张力表达）
  let prev = -Infinity;
  for (let T = 0; T <= 1.001; T += 0.1) {
    const e = bedEnergyDb(bedTargets(st({ T }), sp));
    assert.ok(e >= prev - 1e-9, `T=${T.toFixed(1)} 能量单调不减`);
    prev = e;
  }
});

test('78 床第五态·引带过头（设计二§七.1）：带走照走嘶独止·哼存·crackle 不涉（声籍画外唱机）', () => {
  const brown = bedTargets(st({ T: 0.5 }), sp);              // 棕带常态（leader 缺省=false）
  const lead = bedTargets({ ...st({ T: 0.5 }), leader: true }, sp);
  assert.ok(brown.hiss > 0, '棕带带走=嘶在（缺省兼容·旧调用方零扰）');
  assert.equal(lead.hiss, 0, '引带过头=嘶止（无氧化层物理无嘶——带走照走）');
  assert.equal(lead.hum, brown.hum, '哼存（马达照转·引带不动哼）');
  assert.equal(lead.crackle, brown.crackle, 'crackle 同值（画外唱机的磨损·不涉机上引带）');
  assert.equal(lead.filterHz, brown.filterHz, 'T 滤波律不涉引带');
  // 与暂停态的物理区别：同为嘶止哼存，但引带态 moving 仍真——两态输出同、路径异（状态表第五行独立成立）
  const paused = bedTargets({ ...st({ T: 0.5 }), moving: false }, sp);
  assert.equal(lead.hiss, paused.hiss, '引带与暂停同为嘶止（输出面）');
});

test('79 床第六态·TEST 电源微嗡（设计三§六.1）：哼降变压器级·嘶止·crackle 止——机器醒着屏住呼吸', () => {
  const on = bedTargets(st({ T: 0.5 }), sp);
  const tst = bedTargets({ ...st({ T: 0.5 }), test: true }, sp);
  const ratio = Math.pow(10, sp.bed.testHumDb / 20);
  assert.ok(Math.abs(tst.hum - on.hum * ratio) < 1e-12, `哼降精确 testHumDb（${sp.bed.testHumDb}dB=变压器级）`);
  assert.ok(tst.hum > 0, '微嗡在——机器醒着不是死机');
  assert.ok(sp.bed.testHumDb < 0, '呼吸级之下（参数为负 dB）');
  assert.equal(tst.hiss, 0, 'TEST 带不走=嘶止');
  assert.equal(tst.crackle, 0, 'crackle 止（屏住呼吸·唱片序钳期画外无声）');
  assert.equal(bedTargets({ ...st({ T: 0.5 }), test: false }, sp).hum, on.hum, '缺省/显式 false=旧世界零扰');
});
...
test('㉟ 治理：_ 键不入哈希；改值即改哈希', () => {
  ...
  tweaked.bed.humGain = 0.05;
  assert.notEqual(hashJson(tweaked), h0, '改 humGain 应改哈希（调音抽屉的治理锚）');
});
...
test('㊳ 哼恒稳＋嘶门控（v3 渲染实证）：地板不呼吸（呼吸机器已拆）；moving 假嘶带即无声', () => {
  // 哼包络恒稳：1s 窗摆幅 <1dB（旧呼吸乘法级已随织体床退役——死床即新法）
  const ctx = new OfflineCtx(EAR_SR);
  const eng = buildEngine(ctx, sp, { repoKey: 'golden' });
  eng.setMute('hiss', true); eng.setMute('crackle', true);
  eng.startTransport(0, 1, constTrack(0.3, 0.3), 40000);
  const tapHum = ctx.tap(eng.nodes['humLevel'] as unknown as OfflineNode);
  eng.scheduleGridUntil(20);
  ctx.render(20);
  const tap = tapHum();
  let max = -Infinity, min = Infinity;
  for (let from = 3; from + 1 <= 19; from += 0.5) {
    const w = rmsDb(tap, EAR_SR, from, from + 1);
    if (w > max) max = w; if (w < min) min = w;
  }
  assert.ok(max - min < 1.0, `哼 1s 窗摆幅 ${(max - min).toFixed(2)}dB < 1dB（地板不呼吸）`);
  // 嘶门控渲染实证：同图两渲——moving 由暂停旗压假（pauseRecord 无唱片照设旗）
  const hissRun = (paused: boolean): number => {
    const c2 = new OfflineCtx(EAR_SR);
    const e2 = buildEngine(c2, sp, { repoKey: 'golden' });
    e2.setMute('hum', true); e2.setMute('crackle', true); e2.setMute('fg', true);
    e2.startTransport(0, 1, constTrack(0.3, 0.3), 40000);
    if (paused) e2.pauseRecord(0);
    e2.scheduleGridUntil(12);
    return bandRmsDb(c2.render(12), EAR_SR, 2200, 7500, 6, 12);
  };
  const on = hissRun(false), off = hissRun(true);
  assert.ok(on > -60, `带走嘶在（${on.toFixed(1)}dBFS）`);
  assert.ok(on - off > 20, `暂停抬带嘶止（Δ${(on - off).toFixed(1)}dB > 20）`);
});

test('㊴ stem 定标锁 v3：三 stem（哼/嘶/crackle）渲染 RMS 贴设计电平（CALIB/manifest 漂移即红）', () => {
  ...
  for (const k of ['humLevel', 'hissLevel', 'crackleLevel'] as const) {
  ...
  assert.ok(Math.abs(meas('humLevel') - dB(bt.hum)) <= 1, `哼 ${meas('humLevel').toFixed(2)} vs 设计 ${dB(bt.hum).toFixed(2)}（±1dB·humNorm 定标锁）`);
  assert.ok(Math.abs(meas('hissLevel') - dB(bt.hiss)) <= 1, `嘶 ${meas('hissLevel').toFixed(2)} vs ${dB(bt.hiss).toFixed(2)}`);
...
test('㊺ 三关铁律（v3）：马达低哼自述过关；hissGain>humGain×4 即参数层拦死（床律）', () => {
  const ctx = new OfflineCtx(EAR_SR);
  const eng = buildEngine(ctx, sp, { repoKey: 'golden' });
  ...
  bad.bed.hissGain = bad.bed.humGain * 5;
  assert.throws(() => resolveSoundParams(bad), /床律/, '嘶不得盖哼一个量级——床是地板不是戏');
});
...
test('㊽ 嘶直达路实证（v3）：高 T（lp 收窗至 1.8k）嘶带 RMS 仍贴设计——变闷不滤变糙原法', () => {
  const ctx = new OfflineCtx(EAR_SR);
  const eng = buildEngine(ctx, sp, { repoKey: 'ear:storm', assets: earAssets() });
  eng.setMute('hum', true); eng.setMute('crackle', true); eng.setMute('fg', true);
  eng.startTransport(0, 1, constTrack(0.95, 0.2), 30000);
  eng.scheduleGridUntil(20);
  const tapHiss = ctx.tap(eng.nodes['hissLevel'] as unknown as OfflineNode);
  const wav = ctx.render(20);
  const bandMaster = bandRmsDb(wav, EAR_SR, 2200, 7500, 5, 20);
  const bandStem = bandRmsDb(tapHiss(), EAR_SR, 2200, 7500, 5, 20) + 20 * Math.log10(0.9);   // 同带对照＋master 0.9
  assert.ok(Math.abs(bandMaster - bandStem) <= 0.5, `嘶带 master ${bandMaster.toFixed(1)} vs 直达对照 ${bandStem.toFixed(1)}（±0.5dB）——lp 收窗未吃嘶（hissNorm 系全谱定标·带内对带内才同尺）`);
});
...
test('53 bedTargets recordOn（v3）：唱片在位床整体近隐（P0-2 混音宪法）·近隐但在·缺省=0dB 兼容', () => {
  const off = bedTargets(st({ T: 0.5, A: 0.6 }), sp);
  const on = bedTargets({ ...st({ T: 0.5, A: 0.6 }), recordOn: true }, sp);
  const under = Math.pow(10, (sp.bed.underRecordDb ?? 0) / 20);
  assert.ok(Math.abs(on.hum - off.hum * under) < 1e-12, '哼近隐 = ×dbToLin(underRecordDb)');
  assert.ok(Math.abs(on.hiss - off.hiss * under) < 1e-12, '嘶近隐同闸');
  assert.ok(Math.abs(on.crackle - off.crackle * under) < 1e-12, 'crackle 近隐同闸');
  assert.ok(on.hum > 0 && on.hiss > 0, '近隐但在——状态表"带走=哼+嘶"在唱片下仍真');
  const spLegacy = { ...sp, bed: { ...sp.bed, underRecordDb: undefined } };
  const onLegacy = bedTargets({ ...st({ T: 0.5, A: 0.6 }), recordOn: true }, spLegacy);
  assert.equal(onLegacy.crackle, off.crackle, '缺省=旧判据冻结');
});
...
test('57 唱片链渲染：合成唱片 STUCK 换源＋tape-stop 滑停至静默（全排程纪律的渲染实证）', () => {
  ...
  for (const n of ['hum', 'hiss', 'crackle'] as const) eng.setMute(n, true);   // v3：哼 DONE 不熄（听得见的安静）——唱片链测试须隔离床
...
test('59 针嗒真身（复核庭探针法典化）：静默唱片+全床静音重演卡碟——嗒可测、等距、峰间静默、复走无僵尸', () => {
  ...
  for (const n of ['hum', 'hiss', 'crackle', 'fg'] as const) eng.setMute(n, true); // 唱片链不静音（v3 名单）
...
test('60 hiss 采样率不变性（M2.4 §A.2 漂修执法）：44.1k/48k 带内 RMS 同刻度', () => {
  const bandAt = (sr: number) => {
    ...
    for (const n of ['hum', 'crackle', 'fg', 'record'] as const) eng.setMute(n, true); // 唯 hiss 在场（v3 名单）
