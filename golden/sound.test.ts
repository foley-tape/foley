...
test('53 bedTargets recordOn：作曲四层退场、磨损近隐（P0-2 混音宪法）、未传=R2 旧世界（判据冻结）', () => {
  ...
  // P0-2 混音宪法（LEDGER）：唱片为主声道——磨损随 underRecordDb 近隐（repo params -9dB）
  const under = Math.pow(10, (sp.bed.underRecordDb ?? 0) / 20); // repo params 必有 -9；?? 只为类型面（可选键）
  assert.ok(Math.abs(on.crackle - off.crackle * under) < 1e-12, 'crackle 近隐 = ×dbToLin(underRecordDb)');
  assert.ok(Math.abs(on.hissLin - off.hissLin * under) < 1e-12, 'hiss 近隐同闸');
  // 兼容面：参数缺席＝0dB＝磨损照旧（旧 fixtures/金测试旧世界原样）
